...
int reduction(Search_Local & /* sl */, int /* mv */, int depth, bool /* pv_node */, bool /* in_check */, int searched_size, bool dangerous) {
   ...
   if (depth >= 3 && searched_size >= 3 && !dangerous) {
      red = (searched_size >= 6) ? depth / 2.4 : 1;
   }
...
void command(Scanner & scan) {
   ...
   if (false) {
      ...
      std::cout << "id name Kōhai 1.1" << std::endl;
      std::cout << "id author Fabien Letouzey and Michael Byrne" << std::endl;
      ...
      std::cout << "option name Threads type spin default " << engine::engine.threads << " min 1 max 24" << std::endl;
